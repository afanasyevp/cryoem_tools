...
def main(password, senderEmail, receiverEmails, smtpServer, port, dataPath, timeVar, label, restart, okreport, delay, localhost):
    ...
    #Starting statement
    print(" => Estimation time (in seconds) of the movie number: %f " %
          ((end-start)))
    start = time.time()
    ...
    print(" => ", datetimeStart, "Current data size in TB: ", "{:.3f}".format(
        volume/1000000000000), "Number of movies: ", number, "\n")
    # messages
    messageStart = '''
    ...
    #version: %s''' % (datetimeStart, dataPathAbs, str(timeVar), float(volume/1000000000000), number, ver)
    ...
    for email in receiverEmails:
        ...
        emailSend(port, senderEmail, email, smtpServer,
                  password, messageStart, subject, localhost)
    ...
    while True:
        ...
        print(" => Estimation time (in seconds) of the folder size: %f" %
              (end-start))
        ...
        #if the data collection stops
        if newNumber == number:
            if restart == False:
                statusEmailRestart = ' NOT'
            else:
                statusEmailRestart = ''
            messageError = '''    
            ...
            #This message is sent from Python script 
            #https://github.com/afanasyevp/cryoem_tools/data_collection_alarm.py
            #version: %s''' % (now(), str(timeVar), int(newNumber), float(newVolume/1000000000000), dataPathAbs, ver)
            # print(messageError)
            subject = 'ERROR IN DATA COLLECTION [DO NOT REPLY]'
            for email in receiverEmails:
                emailSend(port, senderEmail, email, smtpServer,
                          password, messageError, subject, localhost)
            if args.restart == False:
                print('''=> ERROR!!! The script is terminated! POTENTIAL ERROR IN THE DATA COLLECTION occured on %s
                No changes in the volume size has been registered in the last %s minutes:
                %i movies collected (occupying %.3f TB) in the %s directory. 
                An email is sent to %s''' % (now(), str(timeVar), int(newNumber), float(newVolume/1000000000000), dataPathAbs, email))
                sys.exit(2)
            else:
            ...
            if newVolume == 0:
                print(" => ", now(), "Current data size in TB: could not be determined (error occured at estimation)",
                      "Number of movies: ", number, "\n => The data_collection_alarm.py script is still running...\n")
            else:
                number = newNumber
                print(" => ", now(), "Current data size in TB: ", "{:.3f}".format(
                    volume/1000000000000), "Number of movies: ", number, "\n => The data_collection_alarm.py script is still running...\n")
                if okreport == True:
                    ...
                    #This message is sent from Python script 
                    #https://github.com/afanasyevp/cryoem_tools/data_collection_alarm.py
                    #version %s''' % (now(), dataPathAbs, str(timeVar), float(newVolume/1000000000000), number, ver)
                    timeCount += 1
                    if timeCount % 7 == 0:  # to make it less often, increase 7 to 14
                        for email in receiverEmails:
                            emailSend(port, senderEmail, email, smtpServer,
                                      password, messageOK, subject, localhost)
            time.sleep(timeInterval)
...
=================================================================================================''' % ver
    ...
    add('--okreport', default=False, action='store_true',
        help="Keep sending reports that the data is collecting every 7*[time interval] - ~2.5 hours for 20 mins interval")
    ...
    add('--die', default=2.5, help="Kill program after this number of days")
    ...
    p = multiprocessing.Process(target=main, name="main", args=(args.password, args.emailfrom, args.to, args.smtp,
                                                                args.port, args.path, float(args.time), args.label, args.restart, args.okreport, args.delay, args.localhost, ))
    p.start()
    if args.to == None:
