...
def make_fsc_plots(curves, units, pix):
    ...
    if len(curves) == 1:
        plt.plot(curves[0]["frac_of_nq"], curves[0]["fsc"], linewidth=1)
    else:
...
def main(args):
    ...
    if software == "cistem":
        for fsc_file in fsc_files:
            if fsc_file[-4:] != ".txt":
                sys.exit(
                    f" => ERROR in checking the type of cisTEM file {fsc_file}! Must be .txt"
                )
        ...
        for fsc_file in fsc_files:
            if fsc_file[-4:] == ".xml":
                ...
                if software == "relion":
                    sys.exit(
                        ' => ERROR! Wrong input format: "--software relion" option accepts .dat and .xml files'
                    )
...
"""
    ...
    add(
        "--software",
        default="relion",
        choices=["relion", "cistem", "cryosparc"],
        help="Options: relion/cryosparc/cistem | From which software the files were exported",
        metavar="",
    )
    ...
    if args.software == "cryosparc":
        for fsc_file in args.i:
            if PurePath(fsc_file).suffix == ".txt" and args.box == None:
                sys.exit(
                    " => ERROR! No --box provided! Please specify your box size for cryosparc .txt file"
                )
